...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const target = await dbGet<{ id: string; full_name: string | null; linkedin_url: string | null; sales_nav_url: string | null }>(
    "SELECT id, full_name, linkedin_url, sales_nav_url FROM targets WHERE id = ?", [id]
  );
  ...
  const account = await resolveLinkedInAccount(id, req.body?.account_id);
