...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === "GET") {
    const account = await dbGet(`SELECT ${ACCOUNT_COLUMNS} FROM accounts WHERE id = ?`, [id]);
  ...
  if (req.method === "PUT") {
    ...
    await dbRun(
      `UPDATE accounts SET
       ...
       WHERE id = ?`,
       [name, email, daily_connection_limit, daily_message_limit, daily_inmail_limit, active_hours_start, active_hours_end, timezone, working_days, id]
    );
    return res.json(await dbGet(`SELECT ${ACCOUNT_COLUMNS} FROM accounts WHERE id = ?`, [id]));
  }
