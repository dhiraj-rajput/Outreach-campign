...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === "PATCH") {
    ...
    if (plan !== undefined) {
      ...
      db.prepare("UPDATE users SET plan = ?, plan_updated_at = datetime('now') WHERE id = ?").run(plan, userId);
    }
    if (role !== undefined) {
      ...
      if (role === "user" && userId === admin.userId) {
        const otherAdmins = db.prepare("SELECT COUNT(*) as n FROM users WHERE role = 'super_admin' AND id != ?").get(userId) as { n: number };
        if (otherAdmins.n === 0) {
