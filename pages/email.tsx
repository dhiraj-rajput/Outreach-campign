...
export default function EmailPage() {
  ...
  const initialTab: MainTab =
    router.query.tab === "health" || router.query.tab === "email-health" ? "health" : "overview";
  ...
  useEffect(() => {
    ...
    if (mainTab === "health") q.tab = "health";
    else delete q.tab;
  ...
  return (
    <>
      ...
      <div className="space-y-5">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-sm text-base-content/40 mt-0.5">
              Campaign &amp; newsletter history, unsubscribes, and per-account send health.
            </p>
          </div>
        ...
        {/* ═══════════════ OVERVIEW TAB (former Email page) ═══════════════ */}
        {mainTab === "overview" && (
          <>
            {loading || !totals ? (
              ...
              <>
                          ...
                          <tr key={c.workflow_id} className="border-t border-base-300/40">
                            ...
                            <td className="py-2 text-success">
                              ...
                              <span className="text-base-content/30">
                                ({c.emails_sent ? Math.round((c.opened / c.emails_sent) * 100) : 0}%)
                              </span>
                            </td>
                      ...
                      <tbody>
                        {editions.map((e) => (
                          <tr key={e.id} className="border-t border-base-300/40">
                            ...
                            <td className="py-2">
                              {e.sent_count}/{e.total_recipients}
                            </td>
                ...
                <div className="bg-base-200 border border-base-300/50 rounded-xl p-5">
                    ...
                    activityFeed.length === 0 ? (
                      ...
                      <ul className="space-y-2">
                        {activityFeed.map((a) => (
                          <li
                            key={`${a.source}-${a.id}`}
                            className="flex items-center gap-3 text-sm py-1.5 border-b border-base-300/30 last:border-0"
                          >
                            <span
                              className={`w-1.5 h-1.5 rounded-full shrink-0 ${
                                a.source === "campaign" ? "bg-warning" : "bg-secondary"
                              }`}
                            />
                            <span className="flex-1 min-w-0 truncate">
                              {a.source === "campaign" ? (
                                <>
                                  ...
                                  {a.workflow_name ? (
                                    <span className="text-base-content/40"> — {a.workflow_name}</span>
                                  ) : null}
                                </>
                              ) : (
                                <>
                                  &quot;{a.edition_title}&quot; sent to{" "}
                        ...
                        {unsubscribed.map((u) => (
                          <tr
                            ...
                            className={`border-t border-base-300/40 ${
                              highlight && u.target_id === highlight ? "bg-error/5" : ""
                            }`}
                          >
                            <td className="py-2">
                              {u.target_id ? (
                                <Link
                                  href={`/contacts/${u.target_id}`}
                  ...
                  <p className="text-[11px] text-base-content/30 mt-4 flex items-center gap-1.5">
                    <RiCheckboxCircleLine size={12} /> Unsubscribed contacts are automatically blocked from
                    being re-added to campaigns or newsletters.
                  </p>
                </div>
              </>
            )}
        ...
        {/* ═══════════════ HEALTH TAB (former Email Health page) ══════════ */}
        {mainTab === "health" && (
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-base-content/50 mt-0.5">
                  Daily send volume per account vs ramp limits
                </p>
              </div>
              <div className="flex items-center gap-3">
                ...
                >
                  <RiRefreshLine size={13} className={healthLoading ? "animate-spin" : ""} />
                  Refresh
                </button>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4 flex-wrap">
              ...
              <div className="px-4 py-3 rounded-xl bg-base-200 border border-base-300/50">
                ...
                <div className="text-2xl font-semibold text-base-content">
                  {health?.accounts.filter((a) => a.sent_today > 0).length ?? 0}
                </div>
              </div>
              {overLimit.length > 0 ? (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-base-300/30">
                      <th className="text-left px-5 py-2.5 text-xs font-medium text-base-content/40 whitespace-nowrap">
                        Account
                      </th>
                      ...
                      {health?.days
                        ...
                        .map((d) => (
                          <th
                            key={d}
                            className="text-right px-4 py-2.5 text-xs font-medium text-base-content/25 whitespace-nowrap"
                          >
                            {formatDay(d)}
                          </th>
                        ))}
                      <th className="text-right px-5 py-2.5 text-xs font-medium text-base-content/40 whitespace-nowrap">
                        Limit today
                      </th>
                    </tr>
                  </thead>
                          ...
                          <td className="px-4 py-3 text-right">
                            <span
                              className={`text-sm font-semibold ${
                                over ? "text-error" : a.sent_today > 0 ? "text-base-content" : "text-base-content/30"
                              }`}
                            >
                              {a.sent_today}
                            </span>
                  ...
                  <tfoot>
                    <tr className="border-t border-base-300/40 bg-base-300/20">
                      ...
                      <td className="px-4 py-2.5 text-right text-sm font-semibold text-base-content">
                        {totalToday}
                      </td>
                      {health?.days
                        .slice(0, -1)
                        .reverse()
                        .map((d) => {
                          const sum = health.accounts.reduce(
                      ...
                      <td className="px-5 py-2.5 text-right text-xs font-medium text-base-content/50">
                        {totalLimit}
                      </td>
                    </tr>
                  </tfoot>
                  ...
                  {health?.recentLogs.map((l, i) => {
                    ...
                    return (
                      <div key={i} className="px-5 py-2.5 flex items-start justify-between gap-4">
                        ...
                        <div className="text-[10px] text-base-content/30 whitespace-nowrap shrink-0">
                ...
                <div className="divide-y divide-base-300/20 max-h-96 overflow-y-auto">
                  {health?.guardTrips.map((g, i) => {
                    const acc = g.email_account_id
                      ? health.accounts.find((a) => a.id === g.email_account_id)
                      : null;
                    return (
                      <div key={i} className="px-5 py-2.5 flex items-start justify-between gap-4">
                        <div>
